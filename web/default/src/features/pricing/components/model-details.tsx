...
function ModelHeader(props: { model: PricingModel }) {
  ...
  const marketplaceDisplay = getMarketplaceDisplay(model, t('Discount'))
  ...
  return (
    <header className='pb-5'>
      <div className='flex flex-wrap items-center gap-2.5'>
        ...
        {marketplaceDisplay.showNew && (
          <span className='rounded-full border border-red-500/20 bg-red-500/10 px-2 py-0.5 text-xs font-semibold text-red-700 dark:text-red-300'>
            NEW
          </span>
        )}
        {marketplaceDisplay.discountLabel && (
          <span className='rounded-full border border-orange-500/25 bg-orange-500/10 px-2 py-0.5 text-xs font-semibold text-orange-700 dark:text-orange-300'>
            {marketplaceDisplay.discountLabel}
          </span>
        )}
        {marketplaceDisplay.discountLabel &&
          marketplaceDisplay.promotionNote && (
            <span className='rounded-full border border-red-500/20 bg-red-500/10 px-2 py-0.5 text-xs font-semibold text-red-700 dark:text-red-300'>
              {marketplaceDisplay.promotionNote}
            </span>
          )}
...
function PriceSection(props: {
  ...
  const originalPriceGroup = getOriginalPriceGroup(model)
  ...
  if (!isTokenBased) {
    const currentPrice = formatFixedPrice(
      model,
      groupKey,
      showRechargePrice,
      priceRate,
    ...
    const originalPrice = originalPriceGroup
      ? formatFixedPrice(
          model,
          originalPriceGroup,
          showRechargePrice,
        ...
        <div className='flex items-baseline justify-between'>
          ...
          <div className='flex items-baseline gap-2'>
            {originalPrice && originalPrice !== currentPrice && (
              <span className='text-muted-foreground/40 font-mono text-xs tabular-nums line-through'>
                {originalPrice}
              </span>
            )}
  ...
  return (
    <section className='border-b py-4'>
      ...
      <div className='space-y-1.5'>
        {items.map((item) => {
          const currentPrice = formatGroupPrice(
            model,
            groupKey,
            item.type,
            tokenUnit,
          ...
          return (
            ...
            >
              ...
              <span className='text-foreground font-mono text-sm tabular-nums'>
                {originalPrice && originalPrice !== currentPrice && (
                  <span className='text-muted-foreground/40 mr-2 text-xs line-through'>
                    {originalPrice}
                  </span>
                )}
                {currentPrice}
