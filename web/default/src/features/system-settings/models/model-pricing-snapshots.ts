...
const getSoraTierCount = (
  tiers?: ModelPricingSnapshot['soraResolutionTiers']
) => normalizeSoraResolutionTiers(tiers || []).length

const getSoraPriceSummary = (
  row: ModelPricingSnapshot,
  t: (key: string) => string
) => {
  ...
  return tierCount > 0
    ? `${t('Fixed price')} $${row.price} / s · ${tierCount} ${t('tiers')}`
    : `${t('Fixed price')} $${row.price} / s`
}

const getSoraPriceDetail = (
  row: ModelPricingSnapshot,
  t: (key: string) => string
) => {
...
export const buildModelSnapshots = ({
  ...
  const soraPricingMap = safeJsonParse<Record<string, SoraPerRequestPricing>>(
    soraPerRequestPricing,
    {
      fallback: {},
...
export const getSnapshotSignature = (snapshot?: ModelPricingSnapshot) => {
  ...
  return JSON.stringify({
    ...
    soraPerRequestPricingEnabled:
      snapshot.soraPerRequestPricingEnabled ?? false,
