...
export const defaultEmailSettingsFormValues: EmailFormValues = {
  SMTPServer: '',
  SMTPPort: '',
  SMTPAccount: '',
  SMTPFrom: '',
...
function formatUserOption(user: MarketingEmailUser, emptyEmailLabel: string) {
  ...
  const email = user.email || emptyEmailLabel
...
export function EmailSettingsSection({
  ...
  const saveOption = async (request: UpdateOptionRequest) => {
    if (updateOptionRequest) {
      const result = await updateOptionRequest(request)
      if (!result.success) {
    ...
    const result = await updateOption.mutateAsync(request)
    if (!result.success) {
      throw new Error(result.message || t('Failed to update setting'))
    }
    return result
  ...
  const onSubmit = async (values: EmailFormValues) => {
    ...
    const orderedUpdates = [...changedUpdates].sort((a, b) => {
      if (a.key === 'EmailDomainWhitelist') return -1
  ...
  const handleSendMarketingEmail = async () => {
    ...
    try {
      const result = await (sendMarketingEmailRequest ?? sendMarketingEmail)({
  ...
  const renderTemplateFields = (group: TemplateGroup) => {
    ...
    return (
      <div className='space-y-5'>
        <div className='bg-muted/25 flex flex-col gap-3 rounded-lg border p-4 sm:flex-row sm:items-start sm:justify-between'>
        ...
        {group.marketing && (
          <div className='bg-muted/25 space-y-4 rounded-lg border p-4'>
              ...
              <div className='flex flex-wrap gap-2'>
                {selectedUsers.map((user) => (
                  <Badge key={user.id} variant='secondary' className='gap-1'>
                    {formatUserOption(user, t('No email'))}
                    <button
                      type='button'
                      className='hover:bg-muted-foreground/10 rounded-sm'
            ...
            <div className='bg-background max-h-56 overflow-y-auto rounded-md border'>
                ...
                userOptions.map((user) => {
                  ...
                  return (
                    <label
                      key={user.id}
                      className={cn(
                        'hover:bg-muted/50 flex cursor-pointer items-center gap-3 border-b p-3 text-sm last:border-b-0',
                        checked && 'bg-muted/60'
                      )}
                    >
                      <Checkbox
        ...
        >
          ...
          <div className='flex flex-wrap items-center gap-3'>
            <Button type='submit' disabled={isSaving}>
              ...
              {isSaving ? t('Saving...') : t('Save Email Settings')}
            </Button>
