...
func isRequestPriceMultiplier(node ast.Node) bool {
	...
	return multiplierOK && fallbackOK && fallback == 1 && multiplier >= 0 &&
		!math.IsNaN(multiplier) && !math.IsInf(multiplier, 0)
}

// validateFixedPricingTree keeps fixed prices as complete tier leaves. This
// prevents accidental token multiplication or ambiguous mixed charges.
