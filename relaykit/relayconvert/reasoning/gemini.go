...
func RenderGemini(model string, intent Intent, maxOutputTokens *uint, adapterBudgetPercentage float64) (GeminiRender, error) {
	...
	if capabilities.kind == geminiThinkingNotConfigurable {
		if !intent.HasStrength() && capabilities.supportsIncludeThoughts {
			return GeminiRender{Config: &dto.GeminiThinkingConfig{IncludeThoughts: boolValue(intent.IncludeThoughts)}, EffectiveEffort: EffortHigh}, nil
		}
	...
	if capabilities.kind == geminiThinkingUnknown {
		...
		return GeminiRender{Config: &dto.GeminiThinkingConfig{IncludeThoughts: boolValue(intent.IncludeThoughts)}}, nil
	}

	config := &dto.GeminiThinkingConfig{IncludeThoughts: boolValue(intent.IncludeThoughts)}
