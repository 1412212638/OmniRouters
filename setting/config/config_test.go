...
func TestUpdateConfigFromMapMapReplacement(t *testing.T) {
	...
	if _, ok := cfg.Modes["model-a"]; ok {
		t.Errorf("Modes still contains model-a after update; got %v", cfg.Modes)
	}
	if _, ok := cfg.Exprs["model-a"]; ok {
		t.Errorf("Exprs still contains model-a after update; got %v", cfg.Exprs)
	}
...
func TestUpdateConfigFromMapEmptyMapClearsAll(t *testing.T) {
	cfg := &testConfigWithMap{
		Modes: map[string]string{"model-a": "tiered_expr"},
		Exprs: map[string]string{"model-a": "p * 5 + c * 25"},
	}
...
func TestUpdateConfigFromMapScalarFieldsUnchanged(t *testing.T) {
	cfg := &testConfigWithMap{
