...
func sessionAsyncPaymentFailed(ctx context.Context, event stripe.Event, callerIp string) {
	...
	if topUp.EffectivePaymentProvider() != model.PaymentProviderStripe {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe 异步支付失败但订单支付网关不匹配 trade_no=%s payment_provider=%s client_ip=%s", referenceId, topUp.EffectivePaymentProvider(), callerIp))
		return
	}
...
func fulfillOrder(ctx context.Context, event stripe.Event, referenceId string, customerId string, callerIp string) {
	...
	topUp := model.GetTopUpByTradeNo(referenceId)
	shouldNotify := topUp != nil &&
		topUp.EffectivePaymentProvider() == model.PaymentProviderStripe &&
