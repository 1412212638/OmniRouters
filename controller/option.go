...
func GetOptions(c *gin.Context) {
	...
	for k, v := range common.OptionMap {
		...
		if isSensitiveOptionKey(k) && !isVisiblePublicKeyOption(k) {
			continue
		}
...
func UpdateOption(c *gin.Context) {
	...
	case "group_ratio_setting.group_model_ratio":
	...
	case "MemberTierRules":
		err = setting.CheckMemberTierRulesJSONString(option.Value.(string))
		if err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": err.Error(),
	...
	case "perf_metrics_setting.excluded_status_codes":
		_, err = operation_setting.ParseHTTPStatusCodeRanges(option.Value.(string))
		if err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
	...
	case "console_setting.model_square_carousel":
		err = console_setting.ValidateConsoleSettings(option.Value.(string), "ModelSquareCarousel")
		if err != nil {
			c.JSON(http.StatusOK, gin.H{"success": false, "message": err.Error()})
			return
		}
