...
func PreWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.RealtimeUsage) error {
	...
	modelGroupRatio, _, _ := ratio_setting.ResolveGroupModelRatio(relayInfo.UsingGroup, modelName, relayInfo.UserId)
...
func PostAudioConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent string) {
	...
	perfmetrics.RecordRelaySample(relayInfo, true, int64(usage.CompletionTokens), int64(uncachedPromptTokens), int64(usage.PromptTokensDetails.CachedTokens))
	})
}
...
func checkAndSendQuotaNotify(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int) {
	gopool.Go(func() {
		...
		if quotaTooLow {
			...
			remainingQuota := logger.FormatQuota(relayInfo.UserQuota)
			...
			if notifyType == dto.NotifyTypeBark {
				...
				values = []interface{}{prompt, remainingQuota}
			} else if notifyType == dto.NotifyTypeGotify {
				content = "{{value}}，当前剩余额度为 {{value}}，请及时充值。"
				values = []interface{}{prompt, remainingQuota}
			} else if notifyType == dto.NotifyTypeEmail {
				prompt, content = common.BuildQuotaWarningMail(remainingQuota, topUpLink, false)
			} else {
				// 默认内容格式，适用于Email和Webhook（支持HTML）
				content = "{{value}}，当前剩余额度为 {{value}}，为了不影响您的使用，请及时充值。<br/>充值链接：<a href='{{value}}'>{{value}}</a>"
				values = []interface{}{prompt, remainingQuota, topUpLink, topUpLink}
			}
...
func checkAndSendSubscriptionQuotaNotify(relayInfo *relaycommon.RelayInfo) {
	gopool.Go(func() {
		...
		prompt := "您的订阅额度即将用尽"
		topUpLink := PaymentReturnURL("/console/topup")
		remainingQuota := logger.FormatQuota(int(remaining))
		...
		if notifyType == dto.NotifyTypeBark {
			...
			prompt, content = common.BuildQuotaWarningMail(remainingQuota, topUpLink, true)
		} else {
			content = "{{value}}，当前剩余额度为 {{value}}，为了不影响您的使用，请及时充值。<br/>充值链接：<a href='{{value}}'>{{value}}</a>"
			values = []interface{}{prompt, remainingQuota, topUpLink, topUpLink}
		}
