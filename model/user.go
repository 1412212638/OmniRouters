...
type User struct {
	Id int `json:"id"`
	// Legacy Save/Updates must not restore a stale authentication version.
	AuthVersion          int64                      `json:"-" gorm:"type:bigint;not null;default:1;column:auth_version;<-:create"`
	...
	ExtraGroups          StringList                 `json:"extra_groups" gorm:"type:text;column:extra_groups"`
...
func GetUsersByIds(ids []int) ([]User, error) {
	...
	err := DB.Omit("password", "access_token").Where("id IN ?", ids).Find(&users).Error
	return users, err
}
...
func (user *User) EditWithTx(tx *gorm.DB, updatePassword bool) error {
	...
	updates := map[string]interface{}{
		...
		"group":        newUser.Group,
		"extra_groups": normalizeStringList([]string(newUser.ExtraGroups)),
