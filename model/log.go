...
func SumUsedQuota(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, channel int, group string) (stat Stat, err error) {
	...
	// type=0 is the frontend's "all types" sentinel.
...
// DeleteOldLog removes all logs older than targetTimestamp in bounded batches.
// It is retained for the legacy synchronous cleanup endpoint.
func DeleteOldLog(ctx context.Context, targetTimestamp int64, limit int) (int64, error) {
	...
	for {
		...
		deleted, err := DeleteOldLogBatch(ctx, targetTimestamp, limit)
		if err != nil {
			return total, err
		}
		total += deleted
