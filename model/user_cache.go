...
func (user *UserBase) GetExtraGroups() []string {
	if user.ExtraGroups == "" {
		return []string{}
	}
	var groups []string
	if err := common.Unmarshal([]byte(user.ExtraGroups), &groups); err != nil {
		common.SysLog("failed to unmarshal user extra groups: " + err.Error())
		return []string{}
	}
	return []string(normalizeStringList(groups))
}
...
// InvalidateUserCache lets upper layers clear stale user authorization data.
func InvalidateUserCache(userId int) error {
...
func updateUserCache(user User) error {
	...
	if err := updateUserExtraGroupsCache(user.Id, user.ExtraGroups); err != nil {
		return err
	}
...
func updateUserExtraGroupsCache(userId int, groups StringList) error {
	...
	return common.RedisHSetField(
		getUserCacheKey(userId),
		"ExtraGroups",
		serializeStringList(groups),
	)
}
