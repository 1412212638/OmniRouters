...
type TaskBillingContext struct {
	...
	FixedQuotas     map[string]int     `json:"fixed_quotas,omitempty"`      // 固定附加额度（已包含分组倍率）
...
func ClaimQuotaForRefund(id int64, expectedQuota int) (bool, error) {
	...
	result := DB.Model(&Task{}).
		Where("id = ? AND quota = ? AND refund_pending = ?", id, expectedQuota, true).
		Updates(map[string]any{"quota": 0, "refund_pending": false})
	if result.Error != nil {
...
func RestoreQuotaAfterFailedRefund(id int64, quota int) (bool, error) {
	...
	result := DB.Model(&Task{}).
		Where("id = ? AND quota = ? AND refund_pending = ?", id, 0, false).
		Updates(map[string]any{"quota": quota, "refund_pending": true})
	if result.Error != nil {
		return false, result.Error
	}
	return result.RowsAffected > 0, nil
